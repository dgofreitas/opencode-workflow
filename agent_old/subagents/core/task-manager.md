        ...
        1. Create directory:
          ...
          3. Create subtask_NN.json for each task:
              ...
              **FRONTEND RULE**: If a task involves UI design, styling, or frontend implementation:
               1. Set `suggested_agent`: "FrontendDeveloper" (or framework variant: "FrontendDeveloperReact", "FrontendDeveloperVue", "FrontendDeveloperAngular")
               2. For UX/wireframe work, set `suggested_agent`: "UXDesigner"
               3. Include `.opencode/context/core/standards/code-quality.md` in `context_files`.
               4. Ensure `acceptance_criteria` includes "Responsive at all breakpoints" and "Accessible (WCAG)".
