...
### Rule: RTK Passthrough Protocol (scope: all_execution) — MANDATORY

When `rtk vitest` (or any `rtk <command>`) prints `[RTK:PASSTHROUGH]` followed by a `[full output: ~/.local/share/rtk/tee/*.log]` path:

1. **NEVER read that `.log` file.** It is huge and will cause context overflow / pipeline freeze. The Read tool is also hard-blocked via `permission.read: "**/rtk/tee/**": "deny"`.
2. **DO NOT re-run the same `rtk` command** hoping for a parser hit — that's a 2-strike violation.
3. **Switch to native runner immediately.** Examples:
   - `rtk vitest <files>` failed → `cd <pkg> && npx vitest run <files> --bail 1`
   - `rtk jest <files>` failed → `cd <pkg> && npx jest <files> --bail`
   - `rtk pytest <files>` failed → `cd <pkg> && pytest <files> -x`
4. **Isolate to ONE file at a time** if the failing batch is large. Bail on first failure.
5. **If still no readable output** → mark `[BLOCKED]`, report in Test Report, move to next inventory item.
